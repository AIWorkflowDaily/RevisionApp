...
if "user_name" not in st.session_state:
    ...
    if st.session_state.current_index >= len(filtered):
        ...
        if not df_results.empty:
            ...
            st.bar_chart(counts)
        st.download_button("Download Results JSON", data=json.dumps(st.session_state.results, indent=2), file_name="results.json")
        ...
        if help_clicked:
            if help_key in st.session_state:
                ...
                with st.spinner("Getting guidance from ChatGPT..."):
                    ...
                    try:
                        res = openai.chat.completions.create(model="gpt-3.5-turbo", messages=[
        ...
        if submit_clicked and response:
            ...
            st.session_state.results.append({
                ...
                "correct": correct, "level": q.get("level", ""), "subject": q.get("subject", ""), "time_taken": time_taken
            })
